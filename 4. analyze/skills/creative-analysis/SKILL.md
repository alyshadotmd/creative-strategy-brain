...
## STEP 0: CLASSIFY THE AD'S JOB (INTERNAL, RUN BEFORE EVERYTHING ELSE)

Like the rest of this framework, this step is internal analysis, not something that shows up in the written note. But it has to run first, before the glance test in Section 2 or the strategic read in Section 3, because every legibility, completeness, and claims judgment below gets keyed to what's found here. Grading a curiosity-click ad or a brand-choice ad against full-sell standards is the single most common way a diagnosis goes wrong: the same missing mechanism, unresolved claim, or absent CTA is a real weakness on one job and exactly the right move on another.

Classify five things before touching Section 2:

**Entry point.** Every ad has an entry point that's either pain or desire. Name it precisely, not "wants better sleep" but "wakes up hot and can't fall back asleep."

**Persona.** Who is this speaking to, by life context, not just demographics? Section 3c does the full write-up; this is the working answer that feeds everything else.

**Awareness stage.** Measure this against the solution category the ad is actually selling, never against an adjacent product or condition the persona already knows well. Knowing a familiar category well doesn't make someone aware of a different category being pitched here; ask what this ad is actually trying to sell, and how much the viewer already knows about that specific thing.

**The presupposition test.** Judge awareness by what the ad requires the viewer to already accept, not by what brand name they'd need to recognize. A brand casually naming itself doesn't make an ad warm-only when the scene itself sells the category and the appeal from zero: a coffee brand's text-message-style ad built around a stolen bag of beans can educate a cold viewer inside the joke without ever explaining what the brand is. The stage only moves warmer when the ad's premise has to already be accepted for the ad to make sense at all, for example a reminder ad that never explains what it's reminding the viewer of, which presupposes the viewer already wants the thing.

When the ad's actual move is a causal surprise, attributing a familiar problem to an unexpected cause inside the product's own category (a bedding brand pinning hair breakage or overheating on the sheets, rather than genetics or the weather), that is educating, not presupposing. Classify it cold even though it states no explicit mechanism, because it is actively building a belief the viewer did not walk in holding.

**Funnel job.** Sort the ad into one of three buckets:

- Full-sell: tries to carry the viewer from hook to purchase decision inside the ad itself. Mechanism completeness, claim backing, and a resolved CTA all matter here.
- Curiosity-click: opens a sharp, short loop and hands the explaining and selling job to the landing page or a retarget sequence. Withheld mechanism or unresolved claim detail is not a flaw here on its own; the loop's sharpness is what matters, and the landing-page dependency this creates is worth naming as a fact rather than a flaw. A paid static or video in this bucket does not need an in-creative CTA either: the unit itself is clickable, and the landing page owns the next step. Only look for an in-creative CTA when the job is actually full-sell.
- Educational self-qualification: helps the viewer decide "is this for me" across a spread of use cases, usually ending in a soft self-select prompt rather than a hard buy CTA. What matters is whether the format itself, not just the copy, lets someone scan and sort themselves in a couple of seconds.

**Distribution context.** Name the ad's assumed context: cold in-feed (a prospecting audience with zero relationship to the brand, everything has to happen inside the creative), partnership or whitelisted (run from a creator's own account, where the creator's bio and audience fill in context the creative doesn't have to build), or organic-to-followers (posted to an existing audience that already has that context). If the creative only works in one of those contexts, name that as a distribution dependency rather than a flaw. A slow-building, low-legibility opening can be a real problem cold in-feed and perfectly fine whitelisted or organic, where the audience already knows who's talking.

**The hard boundary.** All of the above governs how much explaining and proof an ad owes its audience. It governs nothing else. An ad that contradicts itself is broken regardless of stage or job: a visual persona that clashes with the footage it's paired with, a comparison line that implies alternatives it never names, a claim that trails off mid-thought. Withholding an explanation on purpose for a warm audience is leniency; contradicting yourself is not something any amount of leniency buys back.
...
**Format-relative legibility bar.** The couple-of-seconds standard above applies at full strength to statics and to polished, produced-looking ads, where a scroller has no reason to keep watching past a bad first impression. It applies less strictly to process-demo formats where the making-of itself is the hook (a real-time pour, a demonstration, a curiosity-driven build): the process is doing the scroll-stopping work while the product comes into frame naturally partway through, so a product reveal several seconds in is not a legibility failure on its own. Check first whether the format itself is the attention-holding device before flagging a delayed reveal as a fail.

**Tease-arc carve-out.** When the format is a deliberate short tease with a withheld referent, opening ambiguity about the exact brand or category name is acceptable as long as the product's physical form or texture is visible early, even before the brand or explicit category is named. Someone decanting a product into a personal container is a positive integration cue, not a legibility failure; people do this with things they care enough about to make part of their own routine. Don't fail a tease-arc ad on this check just because the branded package or the exact category word is delayed; only fail it if the product's physical form itself is never shown until well past the ad's own tease window.

**Function-legibility substitute.** If what the product does is instantly clear from the headline plus the visual, a missing category noun is not a fail on its own. Grade the viewer's ability to answer "what would this do for me," not only "what is this called." This substitute only applies when the function itself is unambiguous from the first-glance surface; it does not apply when the function is also unclear, or when the visual and headline together leave genuine doubt about what the product does, not just what it's called.
...
#### 2f. Stage-Gated Legibility Expectations

Run Step 0's awareness and job classification before treating anything in this section as a fail. What this section demands depends on the stage the ad's content actually presupposes, not on a fixed checklist applied to every ad equally.

At the product-aware stage, the ad's job is helping the viewer make a choice, and a case to choose the brand on mission, origin, or trust is a legitimate product-aware strategy. A cookware founder letter that makes its case on safety and values, with no product photo, no feature comparison, and no price anchor, is not failing this section; it's doing exactly what a product-aware brand-choice ad is supposed to do. This only applies to the stage the content presupposes: an ad whose hook is built to stop a cold scroll still owes full legibility at full strength, and the orientation-gap floor below is untouched.

**Format-scoped scannability.** The half-second glance test applies at full strength to formats whose job includes stopping a cold glance: headline statics, comparison layouts, curiosity statics. Long-read formats, like a founder letter, are graded on whether they earn the read from the audience Step 0 identifies, not on whether they can be parsed in half a second. A long-form piece that reads as "extremely simple, quick to read once you commit to it" is that format working, not a glance-test failure. Cold-scoped ads keep the full glance test regardless of format.

#### 2g. Device Semantics

When a static or video puts copy inside a container device (a speech bubble, a chat thread, a reply-style box), that shape carries its own implied meaning: a real message, a complete attributed thought. If the content placed inside it isn't actually conversational or attributed, the shape adds a comprehension cost of its own, separate from whether the language itself sounds authentic. This only applies to ads that actually use a container device; it isn't a check to run on every ad.
...
Refine here the working classification from Step 0. Measure it against the solution category actually being sold, not an adjacent product the persona already knows, using the presupposition test and the reframe exception described there.
...
Scope this against the funnel job and distribution context named in Step 0. A curiosity-click or educational self-qualification ad is not strategically weak just because it doesn't behave like a full-sell ad; judge it against the job it's actually doing. What the hard boundary in Step 0 never excuses, at any stage or any job, is internal incoherence: a persona clash, a dangling comparison, or a half-finished claim.
...
At least one of these needs to be firing for the hook to work. Name which are present and which are missing. If the ad's funnel job (Step 0) is curiosity-click or educational self-qualification, run this alongside the orientation-gap and mechanism-gap distinction in Section 5's Two-Gap Rule: a curiosity ad can pass this test on the strength of a well-managed open loop even while withholding the mechanism entirely.
...
#### 4i. Two-Part Hook Reinforcement (Video)

When a video hook has two clauses, an activity plus a condition ("try these hacks" plus "for when it's too hot to sleep"), check that each clause has at least one visual carrier. A clause riding on wardrobe or setting alone is weak. The fix is usually a small, in-register b-roll beat (a hand fanning, a fan turning) rather than a rewrite. This only applies when the hook genuinely has two distinct clauses; a single-clause hook isn't failed for carrying one idea visually.
...
**The Two-Gap Rule.** For a deliberate curiosity-click or educational self-qualification ad (Step 0), there are two distinct gaps a viewer can be left holding, and they get judged differently. The orientation gap (what category is this and what's roughly being sold) has to close within about a second; curiosity never buys this back, and "only just clear enough" is a fail regardless of how sharp the rest of the ad is. The mechanism gap (how or why it works) can stay open on purpose, and gets graded by the specificity of the question it leaves the viewer holding: a specific, product-directed question ("okay, so how does it actually do that?") is a pass, and diffuse confusion ("wait, what am I even looking at?") is a fail. Do not flag a curiosity or self-qualification ad for withholding mechanism just because it's unresolved; grade the sharpness of the loop instead, and note the landing-page dependency this creates as a fact, not a flaw.
...
**Claim rigor scales with audience temperature.** How much backing a claim needs scales with the awareness stage identified in Step 0. For a cold or problem-aware audience, apply full rigor: absolute and superiority claims need real backing, and a boundary-pushing transformation claim is the biggest flag. For a warm, solution-aware, product-aware, or most-aware audience, treat an unbacked absolute as a reminder or confidence phrasing, and grade it only on whether the delivery would trigger a skeptical viewer, not on whether it carries a citation.

**Claim-object concreteness comes before backing.** Before asking whether a claim is backed, ask what kind of object the claim is actually about. A concrete, familiar object with a hedge (a specific percentage, a coverage number, "up to X") can pass cold without a citation, because a skeptical reader can picture and evaluate the object itself. A vague, abstract object ("wellness," "energy," "clean ingredients") fails even with a soft, modest verb like "supports," because there's nothing concrete for rigor to attach to. When a claim like this fails, the fix is to make the object concrete, not to bolt on a citation.
...
**Register does not equal ad-speak.** When copy reaches for elevated or literary word choice, don't flag it as marketing-speak automatically. Ask what persuasive work the word choice is doing. A cookware founder letter that chooses "in your home" instead of "in your kitchen" is doing real work, widening the stakes of a safety claim by reminding the reader that home, not just the kitchen, is what's at risk. Elevated language with no identifiable job behind it, vague mission-statement phrasing that isn't reinforcing anything specific, still flags.
...
If the ad's funnel job (Step 0) is curiosity-click or educational self-qualification, it does not owe a hard in-creative CTA at all. The ad unit itself is clickable, and the landing page carries the next step. Only flag a missing or weak CTA when the identified job is full-sell.
...
- **Premise believability.** Sanity-check each contrast line as a believable complaint the persona from Step 0 would actually make. A technically clean line that no real person would actually say ("smells icky") undermines the whole comparison even when nothing in it is factually false; a lived, specific complaint ("smells cheap" versus "smells luxurious") does the same job more convincingly. This is advisory unless a line is clearly false; the fix is usually to swap the weak line for one the persona would plausibly say.
...
Before proposing any fix, re-check the ad's job from Step 0 and confirm the thing about to be flagged is actually broken under that job, not just different from what a full-sell ad would do. Never propose a fix for something Step 0's classification says isn't broken. On an ad that's already working, prefer additive enhancements, an extra reinforcing beat, a persistent headline layer, over repairs that imply something is wrong. A good ad doesn't need a rescue; it can still take a small addition. State fixes as findings only: what's wrong or missing, and the concrete change. No effort ratings, no lift estimates, no numeric scores on any part of an ad, in this section or anywhere else in the analysis, the diagnosis is complete when it names the mechanism and the fix, not when it's been scored.
