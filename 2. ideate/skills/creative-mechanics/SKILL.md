---
name: creative-mechanics
description: Reference library of creative mechanics — the structural patterns that decide how an ad builds meaning between its hook, its visuals and its narrative. Use when a concept has an angle and a format but no structural idea, when someone describes an ad and wants to understand or rebuild what made it work, or when a brief needs more than "say this in this format". Sits between messaging angles and visual formats in the creative engine: the angle says what is argued, the format says how it is produced, the mechanic is the cognitive move that makes it land.
---
...
Reads the angle from `2. ideate/messaging-angles/` and the format from `2. ideate/visual-formats/`; the mechanic is the third choice between them. Feeds the concept's `overview.md` in `pipeline/`, and the brief that follows it.

Format and mechanic can be picked in either order. A concept with a format and no mechanic is a production spec; a concept with a mechanic and no format is an idea nobody can shoot.
...
**The same mechanic can be executed across multiple visual formats.**
The Implied Answer works as a lifestyle video, a static image, a talking head, or a carousel. The mechanic travels. The format is just the vessel.
...
The messaging angle defines the **core truth.**
The mechanic defines **how the ad makes the viewer arrive at that truth.**
The hook is the **opening line that triggers the mechanic.**
The visual format is **the production structure that delivers it.**
