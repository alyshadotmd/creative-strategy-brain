...
*Library file, not a skill. `hook-writing` reads this to choose a frame before writing a line.*

A tactic is the frame a hook uses — the shape of the move. The psychological trigger is the mechanism inside that frame, and the voice patterns in `openers-swipe.md` are the sentence structures that carry it. Pick the tactic from the persona's awareness stage and trigger moment, never from what sounds good in isolation.
